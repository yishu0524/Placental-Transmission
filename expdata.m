...
hold off
title('Viral Load Data from mother Macaque')
xlabel('Time t (days)')
ylabel('Viral Load (Log Scale)')
...
plot(t,Monkey3_VL) 
hold on
plot(Monkey3.days,Monkey3.viralLoad,'o')
hold off
title('Viral Load Data from mother Macaque')
xlabel('Time t (days)')
ylabel('Viral Load')
%legend('Interpolation (pchip)','Data')
%%
s1=t;
s2=Monkey3_VL;
p=0.4/D;
...
%hold off
ylim([0 inf])
title('Viral flux on the infant side over time')
xlabel('Time t (days)')
ylabel('Viral flux on the infant side')
...
ax.YDir="reverse"
hold off
title('Placental Tranport of CMV')
%%
temp=[transpose(t),Q]
%%
